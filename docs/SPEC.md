...
- **No authentication, so no per-user state.** Conversations and pins live in `localStorage`, per browser rather than per user, and the query log carries no `user_id` — the shipped table omits the column §8.1 designs because there is nothing truthful to write in it. This also caps the coverage loop: §8.5 ranks gaps by `distinct_users × log(question_count)`, which without identity degrades to raw frequency
